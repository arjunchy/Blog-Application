...
export const loginUser = async (req, resp) => {
    ...
    try {
        ...
        if (match) {
            ...
            return resp.status(200).json({ accessToken: accessToken, refreshToken: refreshToken, name: user.name, username: user.username });

        } else {
            return resp.status(400).json({ msg: "Password does not match" })
        }
    } catch (err) {
        return resp.status(500).json({ msg: "Error while logging in" })
    }
